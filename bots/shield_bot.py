...
def protect(bot, update, args):
    ...
    if len(args) < 5:
        if otype == 's':
            ...
            result = bittrex.buy_limit(coin,quantity/rate,rate)
        order_id = result['result']['uuid']
    else:
        target = float(args[4])
        if otype == 's':
            ...
            result = bittrexv2.trade_buy(market=coin,order_type='LIMIT',quantity=quantity/rate,rate=rate,target=target,condition_type='GREATER_THAN',time_in_effect='GOOD_TIL_CANCELLED')
        order_id = result['result']['OrderId']
    message = "{}".format(order_id)
    bot.send_message(chat_id=my_chatid, text=message,parse_mode=ParseMode.MARKDOWN)
